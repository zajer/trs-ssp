...
let main argv =
    if Array.length argv <> 7 then
        ...
        *)
        ...
        let resultWalks,is_found = Frontend.search_for_walks_leading_to_state init_sm tm dest_state_idx maxNumOfSteps
        if is_found then 
            (
                ...
                //let result_walks = Frontend.walk_from_situation_matrix resultStrategy situationsMx dest_state_idx |> Array.ofList
                Array.Parallel.iteri 
                                (
                                    fun i w -> 
                                        let tf = Frontend.export_walk w importedTransFuns
                                        Frontend.export_trans_funs tf (outputFile+"_"+i.ToString()+".csv")
                                )
                                walksAsArray
                //let walks_to_save = Array.Parallel.map (fun w -> Frontend.export_walk w importedTransFuns) (Array.ofList resultWalks)
